...
def train_model(train_loader, val_loader, num_epochs, MODELO, palavroto, MM):
    ...
    model.fc = nn.Sequential(
        nn.Linear(num_ftrs, 512),  # Adicione uma camada fully connected intermediária, se desejar
        nn.ReLU(),  # Adicione a ativação desejada
        nn.Dropout(0.5),  # Camada de dropout com probabilidade de 50% (0.5)
        nn.Linear(512, 2)  # Camada fully connected de saída
    )
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)
    ...
    # Mostra a perda do treino e validação
    plt.figure(figsize=(10, 5))
